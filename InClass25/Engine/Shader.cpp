...
bool Shader::Load(const WCHAR* fileName, const D3D11_INPUT_ELEMENT_DESC* layoutArray, int numLayoutElements)
{
    ...
    if (LoadShader(fileName, "VS", "vs_4_0", pBlobVS))
    {
        ...
        {
            HRESULT hr = mGraphics->GetDevice()->CreateVertexShader(pBlobVS->GetBufferPointer(), pBlobVS->GetBufferSize(), nullptr, &mShaderVS);
            DbgAssert(hr == S_OK, "Failed to create vertex shader");
            hr = mGraphics->GetDevice()->CreatePixelShader(pBlobPS->GetBufferPointer(), pBlobPS->GetBufferSize(), nullptr, &mShaderPS);
            DbgAssert(hr == S_OK, "Failed to create pixel shader");
            if (nullptr != mShaderVS && nullptr != mShaderPS)
            {
                hr = mGraphics->GetDevice()->CreateInputLayout(layoutArray, numLayoutElements, pBlobVS->GetBufferPointer(), pBlobVS->GetBufferSize(), &mInputLayout);
